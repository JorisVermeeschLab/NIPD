...
def write_sub_script_qsub(taskscriptdir, taskname, taskworkdir, mpinum, stderrdir):
    '''
    ...
    '''
    qsub = '/cm/shared/apps/sge/6.2u5p2/bin/lx26-amd64/qsub'
    ...
    with open(task_qsub, 'w+') as fo:
        ...
        for script in os.listdir(taskscriptdir):
            tasksearch = taskname + '.task.sh'
            if script.endswith(tasksearch):
                if mpinum > 1:
                    myqsub = '%s -wd %s -e %s -o %s -pe openmpi %s %s\n' % (qsub, taskworkdir, stderrdir, logdir, mpinum, script)
                    fo.write(myqsub)
                else:
                    myqsub = '%s -wd %s -e %s -o %s %s\n' % (qsub, taskworkdir, stderrdir, logdir, script)
                    fo.write(myqsub)
    return(task_qsub)
...
def write_sub_script_fastqc():
    ...
    for sampleid in SampleIDs:
        ...
        samplescript = fastqc_scriptdir + '/' + sampleid + '.fastqc.task.sh'
    ...
    write_sub_script_qsub(fastqc_scriptdir, 'fastqc', fastqc_workdir, 1, logdir)
...
def write_sub_script_rename():
    ...
    for sampleid in SampleIDs:
        ...
        samplescript = UMI_scriptdir + '/' + sampleid + '.umimodname.task.sh'
    ...
    write_sub_script_qsub(UMI_scriptdir, 'umimodname', UMI_workdir, 1, logdir)
...
def write_sub_script_align():
    ...
    for sampleid in SampleIDs:
        ...
        samplescript = align_scriptdir + '/' + sampleid + '.align.task.sh'
    ...
    write_sub_script_qsub(align_scriptdir, 'align', align_workdir, 4, logdir)
...
def write_sub_script_coverage():
    ...
    for sampleid in SampleIDs:
        ...
        samplescript = cov_scriptdir + '/' + sampleid + '.cov.task.sh'
        samplescript2 = cov_scriptdir + '/' + sampleid + '.readcount.task.sh'
        ...
        with open(samplescript2, 'w+') as fo2:
            ...
            readcountall_task = '%s -Xmx4G -XX:-UsePerfData -XX:-UseParallelGC -Djava.io.tmpdir=%s -jar %s -T CountReads -R %s -I %s > %s/%s\n' % (java, tmpdir, gatk, reference, infilteredbam, cov_sub_workdir, readcountall)
            fo2.write(readcountall_task)
            #count on target reads
            readcountontar = '%s.sorted.merged.md.filtered.readcountOnTar.out' % (sampleid)
            readcountontar_task = '%s -Xmx4G -XX:-UsePerfData -XX:-UseParallelGC -Djava.io.tmpdir=%s -jar %s -T CountReads -R %s -I %s -L %s > %s/%s\n' % (java, tmpdir, gatk, reference, infilteredbam, target, cov_sub_workdir, readcountontar)
            ...
            readcountontar50_task = '%s -Xmx4G -XX:-UsePerfData -XX:-UseParallelGC -Djava.io.tmpdir=%s -jar %s -T CountReads -R %s -I %s -L %s > %s/%s\n' % (java, tmpdir, gatk, reference, infilteredbam, targetflank50, cov_sub_workdir, readcountontar50)
            fo2.write(readcountontar50_task)

            readcountontar100 = '%s.sorted.merged.md.filtered.readcountOnTar.100bpflank.out' % (sampleid)
            readcountontar100_task = '%s -Xmx4G -XX:-UsePerfData -XX:-UseParallelGC -Djava.io.tmpdir=%s -jar %s -T CountReads -R %s -I %s -L %s > %s/%s\n' % (java, tmpdir, gatk, reference, infilteredbam, targetflank100, cov_sub_workdir, readcountontar100)
            fo2.write(readcountontar100_task)

    write_sub_script_qsub(cov_scriptdir, 'cov', cov_workdir, 1, logdir)
    write_sub_script_qsub(cov_scriptdir, 'readcount', cov_workdir, 1, cov_workdir)
...
def write_sub_script_varcalling():
    ...
    samplescript = var_scriptdir + '/' + familyID + '.haplotypecaller.task.sh'
    ...
    write_sub_script_qsub(var_scriptdir, 'haplotypecaller', var_workdir, 1, logdir)
...
def write_sub_script_AE():
    ...
    for sampleid in SampleIDs:
        ...
        samplescript = AERC_scriptdir + '/' + sampleid + '.aerc.task.sh'
    ...
    write_sub_script_qsub(AERC_scriptdir, 'aerc', AERC_workdir, 1, logdir)
