...
def get_familyinfo(infolist):
    familyinfo = dict()
    
    try:
        with open(infolist, 'rU') as fh:
            for line in fh:
                (familyID, sampleID, paternalID, maternalID, sex, phenotype, trio, noteinfo) = line.strip().split()
                familyinfo[trio] = sampleID

    except IOError:
        print('Fail to open InfoList')
        
    return(familyinfo)


def get_familyID(infolist):
    try:
        with open(infolist) as fh:
            line1 = fh.readline()
            familyID = line1.split('\t')[0]
